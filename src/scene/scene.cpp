...
void Scene::simulation_update()
{
    ...
    auto current_time = steady_clock::now();
     // 计算自上次模拟更新以来经过的时间
    duration frame_duration = current_time - last_update;
    ...
    while (remained_duration>= (duration)time_step) {
        ...
        // 更新最后一次调用 simulation_update 的时间 (last_update)
        last_update = last_update+std::chrono::duration_cast<std::chrono::milliseconds>((duration)time_step);
        //std::chrono::time_point<std::chrono::steady_clock, std::chrono::duration<float>> new_time_point = last_update + (duration)time_step;
        //auto last_update = new_time_point;
    }
}