...
void Toolbar::layout_mode(Scene& scene)
{
    if (ImGui::BeginTabItem("Layout")) {
        ...
        if (selected_object != nullptr) {
            ...
            Quaternionf qx(cos(radians(x_angle) / 2), sin(radians(x_angle) / 2), 0, 0);
            Quaternionf qy(cos(radians(y_angle) / 2), 0, sin(radians(y_angle) / 2), 0);
            Quaternionf qz(cos(radians(z_angle) / 2), 0, 0, sin(radians(z_angle) / 2));
